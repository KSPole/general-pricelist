...
def render(filtered_products, options_df, rk, cat_no_space):
    ...
    if cat_no_space == "천장형브라켓":
        ...
        if sel_out and sel_in:
            ...
            box_h = 60 if box_height_state == "높이 60mm" else 120
            min_len, max_len = 0, 0
            
            if cam_type_state in ["뷸렛카메라", "돔카메라"]:
        ...
        with st.expander("🎯 맞춤 규격 추천기 (최소 / 최대 조절 길이를 입력하세요)", expanded=True):
            ...
            if t_min > 0 and t_max > 0:
                ...
                if valid_combos:
                    ...
                    top_combos = valid_combos[:3]
                elif approx_combos:
                    approx_combos.sort(key=lambda x: (x['penalty'], 0 if x['outer'] == 600 else 1, x['extra'], -x['outer']))
                    top_combos = approx_combos[:3]
                    is_approx = True
                
                if top_combos:
                    if is_approx:
                        st.markdown("<div style='color:#e67e22; font-size:15px; font-weight:bold; margin-top:10px; margin-bottom:5px;'>💡 추천 조합 Top 3 (근사치) - 원하시는 규격을 클릭하세요</div>", unsafe_allow_html=True)
                    else:
                        st.markdown("<div style='color:#d9534f; font-size:15px; font-weight:bold; margin-top:10px; margin-bottom:5px;'>💡 최적 추천 조합 Top 3 - 원하시는 규격을 클릭하세요</div>", unsafe_allow_html=True)
        ...
        with opt_col:
            ...
            if cam_type in ["뷸렛카메라", "돔카메라"]:
                ...
                if sel_out and sel_in:
                    outer_val = int(float(sel_out))
                    inner_val = int(float(sel_in))
                    box_h = 60 if box_height == "높이 60mm" else 120
                    
                    if inner_val >= outer_val:
                        min_len_box = outer_val + (inner_val - outer_val) + (box_h + 60)
                        max_len_box = outer_val + (inner_val - 80) + box_h
                    else:
                        min_len_box = outer_val + box_h
                        max_len_box = outer_val + (inner_val - 80) + box_h
                        
                    st.markdown(f"<div style='font-size:13px; font-weight:bold; color:#d9534f; margin-top:5px; margin-bottom:10px;'>💡 천장에서 최소 {min_len_box}mm ~ 최대 {max_len_box}mm 조절 가능합니다.</div>", unsafe_allow_html=True)
