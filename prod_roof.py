...
def render(filtered_products, options_df, rk, cat_no_space):
    # 💡 [핵심] 규격 입력 여부와 상관없이 무조건 옵션과 이미지를 띄웁니다.
    is_main_ready = True 
    base_price = 0
    product_specs = ""
    ...
    if roof_base == "i형":
        st.markdown("<div style='font-size:14px; margin-top:5px; margin-bottom:5px; color:#555;'>👉 i형 브라켓 규격 입력 <span style='font-size:13px; color:#888;'>(선택사항)</span></div>", unsafe_allow_html=True)
        ...
        if i_len is not None and i_len > 0:
            ...
            if not row.empty:
                ...
                product_specs = f"i형 / 가로: {int(i_len)}mm (단가표 초과 - 협의)"
        else:
            product_specs = "i형 / 규격 미입력 (사이즈 협의)"
        ...
        st.markdown(f"<div style='font-size:14px; margin-top:5px; margin-bottom:5px; color:#555;'>👉 {roof_g_type} 규격 입력 <span style='font-size:13px; color:#888;'>(선택사항)</span></div>", unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        ...
        if g_w is not None and g_h is not None and g_w > 0 and g_h > 0:
            ...
            if not row.empty:
                ...
                product_specs = f"ㄱ형 ({roof_g_type}) / 세로: {int(g_h)}mm x 가로: {int(g_w)}mm (단가표 초과 - 협의)"
        else:
            product_specs = f"ㄱ형 ({roof_g_type}) / 규격 미입력 (사이즈 협의)"

    st.markdown("<hr style='margin: 15px 0px;'>", unsafe_allow_html=True)
    opt_col, img_col = st.columns([5.5, 4.5])
    
    # 💡 [핵심] 카메라 형태를 선택하기 전에는 무조건 "직결" 이미지가 뜨도록 기본값 설정
    cam_suffix = "직결" 
    ...
    with opt_col:
        ...
        # [ㄱ형 추가 옵션]
            ...
            # 💡 [핵심] "난간 샌드위치형"도 다른 옵션들과 동일하게 엑셀에서 단가를 불러오도록 추가
            if roof_g_type != "기본형":
                p = 0
                ...
                elif roof_g_type == "난간 샌드위치형": p = get_opt_price("난간샌드위치형")
                
                if p > 0: 
                    priced_options.append({"cart_name": f"{roof_g_type} (1EA)", "display_name": f"형태: {roof_g_type}", "unit_price": p, "qty_per_main": 1, "qty": 1, "total_per_main": p, "group": "ㄱ형형태"})
                else: 
                    zero_options.append({"cart_name": roof_g_type, "display_name": f"형태: {roof_g_type}", "qty": 1, "qty_per_main": 1})
            ...
            if roof_g_type == "기본형":
                ...
                st.info("💡 난간 구조에 대하여 담당자와 별도 치수 협의가 필요할 수 있습니다.")
    ...
    # -------------------------------------------------------------------------
    # 💡 옥상 브라켓 이미지 파일명 정밀 생성 로직 (직결 이미지가 기본 연동됨)
    # -------------------------------------------------------------------------
    combo_names = []
    ...
    if base_price == 0:
        st.info("💡 가로/세로 규격을 입력하시면 정확한 단가가 적용됩니다.")
