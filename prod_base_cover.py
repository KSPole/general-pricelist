...
def render(filtered_products, options_df, rk, cat_no_space):
    ...
    p_list.append("주문형 베이스커버 (직접 입력)")
    sel_prod = st.selectbox("상세 규격 선택", options=p_list, index=None, placeholder="규격을 선택해주세요", key=f"p_{rk}")

    if sel_prod == "주문형 베이스커버 (직접 입력)":
        st.markdown("<div style='background-color:#f9f9f9; padding:10px; border-radius:5px;'>", unsafe_allow_html=True)
        st.markdown("<div style='font-size:14px; font-weight:bold; color:#d9534f; margin-bottom:8px;'>👇 주문형 규격 정보 입력</div>", unsafe_allow_html=True)
        pipe_val = st.text_input("👉 파이프 지름 (모를 경우 둘레 입력)", placeholder="예: 지름 165 또는 둘레 500", key=f"bc_pipe_{rk}")
        base_val = st.text_input("👉 베이스판 사이즈 (지름 또는 가로x세로 입력)", placeholder="예: 지름 250 또는 200x200", key=f"bc_base_{rk}")
        st.markdown("</div>", unsafe_allow_html=True)
        
        if pipe_val and base_val:
            base_price = 0
            for _, r in filtered_products.iterrows():
                if "주문형" in str(r.get('제품명', '')):
                    base_price = int(r.get('단가', 0))
                    break
            product_specs = f"주문형 베이스커버 / 파이프: {pipe_val} / 베이스판: {base_val}"
            is_main_ready = True
            combo_names = ["베이스커버-주문형", "베이스커버"]
        ...
        if pd.notna(img_val):
            ...
            combo_names = [str(img_val).strip(), "베이스커버"]
        else:
            combo_names = ["베이스커버"]
