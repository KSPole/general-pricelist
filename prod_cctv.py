    ...
    if is_main_ready:
        ...
        with opt_col:
            ...
            if arm_type == "기본형(I형)" or (arm_type == "벽부형" and wall_has_arm == "적용 안 함"):
                ...
                if show_cam:
                    ...
                    cam_main = st.radio("메인 폴 상부에 설치할 카메라 형태", main_cam_opts, index=0, horizontal=True, key=f"cam_main_wall_arm_{rk}", label_visibility="collapsed")
                    if cam_main != "설치 안 함": main_part = render_custom_cctv_camera_parts(cam_main, "메인 폴 상부", f"main_wall_arm_{rk}", allow_40a)
                ...
                if cam_main != "설치 안 함": main_part = render_custom_cctv_camera_parts(cam_main, "메인 폴 상부", f"main_{rk}", allow_40a)
        ...
        if arm_type == "벽부형":
            if wall_has_arm == "ㄱ형 암 적용":
                ...
                base_prefix = f"{cat_no_space}-{arm_kw}-{main_k}-{arm_k}{prod_method_kw}"
                ...
                if main_cam_kw:
                    base_cctv = f"{base_prefix}-{main_cam_kw}{prod_method_kw}"
                    combo_names.append(base_cctv)
                else:
                    base_cctv = f"{base_prefix}{prod_method_kw}"
                    combo_names.append(base_cctv)
